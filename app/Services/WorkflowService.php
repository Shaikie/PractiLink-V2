...
class WorkflowService
{
    public function startFor(Application $application): ApplicationWorkflow
    {
        return DB::transaction(function () use ($application) {
            ...
            $definition = WorkflowDefinition::where('is_active', true)
                ->where(function ($q) use ($application) { $q->where('training_type_id', $application->applicationWindow->training_type_id)->orWhereNull('training_type_id'); })
                ->with(['versions' => fn ($q) => $q->where('status', 'PUBLISHED')->latest('version')])
                ->get()->first(fn ($d) => $d->versions->isNotEmpty());
            ...
            $stage = $version->stages()->orderBy('stage_order')->first();
    ...
    public function transition(ApplicationWorkflow $workflow, string $action, ?User $actor, ?string $comment = null): ApplicationWorkflow
    {
        return DB::transaction(function () use ($workflow, $action, $actor, $comment) {
            ...
            $transition = WorkflowTransition::where('workflow_version_id',$workflow->workflow_version_id)->where('from_stage_id',$workflow->current_stage_id)->where('action',$action)->first();
            if (! $transition) throw ValidationException::withMessages(['transition'=>'This action is not allowed from the current workflow stage.']);
            if ($transition->requires_comment && blank($comment)) throw ValidationException::withMessages(['comment'=>'A comment is required for this transition.']);
            if (! $actor) abort(403,'Authentication is required for workflow actions.');
            if ($workflow->currentStage->required_permission && ! $actor->hasPermission($workflow->currentStage->required_permission)) abort(403,'You are not authorized for the current workflow stage.');
