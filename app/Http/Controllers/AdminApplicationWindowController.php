...
class AdminApplicationWindowController extends Controller
{
    ...
    {
        $validated = $this->validateWindow($request, $applicationWindow);
    ...
    public function destroy(ApplicationWindow $applicationWindow)
    {
        ...
        $old = $applicationWindow->toArray(); $applicationWindow->delete();
    ...
    private function validateWindow(Request $request, ?ApplicationWindow $existing = null): array
    {
        ...
        $open = Carbon::createFromFormat('Y-m-d\\TH:i', $validated['opens_at']);
        $close = Carbon::createFromFormat('Y-m-d\\TH:i', $validated['closes_at']);
        ...
        $overlap = ApplicationWindow::where('training_type_id', $validated['training_type_id'])
            ->when($existing, fn ($q) => $q->whereKeyNot($existing->id))
            ->where('opens_at', '<', $close)
            ->where('closes_at', '>', $open)
            ->exists();
        ...
        $validated['opens_at'] = $open;
        $validated['closes_at'] = $close;
