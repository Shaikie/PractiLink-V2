...
class StudentApplicationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('student.applications.show', $application)->with('success', 'Application draft saved.');
    }
    ...
    public function submit(Application $application, WorkflowService $workflows)
    {
        ...
        foreach ($requiredFields as $field) {
            if (blank($application->{$field})) {
                throw ValidationException::withMessages([$field => 'This field is required before submission.']);
            }
        }
        abort_unless($application->current_study_year >= 1 && $application->current_study_year <= 20, 422, 'Current study year is invalid.');
        ...
        $required = DocumentType::where('is_active', true)->where('is_required', true)->pluck('id');
