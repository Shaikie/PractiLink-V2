...
class AdminStaffController extends Controller
{
    ...
    public function update(Request $request, User $staff)
    {
        ...
        $retainsAdministrator=in_array(Role::where('slug','administrator')->value('id'),$data['roles'] ?? [],true);
        ...
        if ($currentlyAdministrator && (!$retainsAdministrator || empty($data['is_active']))) {
            $otherActiveAdmins=User::whereKeyNot($staff->id)->where('is_active',true)->whereHas('roles',fn($q)=>$q->where('slug','administrator'))->exists();
