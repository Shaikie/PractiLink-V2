...
class AdminPlacementController extends Controller
{
    ...
    public function store(Request $request, Application $application, ApplicationLifecycleService $lifecycle)
    {
        $this->authorizeCto($request->user());
        abort_unless($application->status === 'ACCEPTED', 422, 'Only approved applications can be placed.');
        abort_if($application->placement()->exists(), 422, 'This application already has a placement.');
        ...
        if (!$this->eligibleSupervisors()->whereKey($data['supervisor_user_id'])->exists()) {
        ...
        DB::transaction(function () use ($data, $application, $request, $lifecycle): void {
            $locked = Application::query()->whereKey($application->id)->lockForUpdate()->with('workflow.currentStage')->firstOrFail();
            abort_unless($locked->status === 'ACCEPTED' && !$locked->placement()->exists(), 422, 'This application already has a placement or is no longer approved.');
            abort_unless($locked->workflow?->currentStage?->code === 'CTO_PLACEMENT', 422, 'This application is no longer at the CTO placement stage.');
        ...
        return redirect()->route('admin.placements.index')->with('success', 'Placement allocated, supervisor assigned, and the application workflow is complete.');
    }
    ...
    public function updateStatus(Request $request, Placement $placement)
    {
        ...
        $allowedTransitions=['ALLOCATED'=>['ACTIVE','CANCELLED'],'ACTIVE'=>['COMPLETED','CANCELLED'],'COMPLETED'=>[],'CANCELLED'=>[]];
        ...
        DB::transaction(function () use ($placement,$data,$request,$allowedTransitions): void {
            $placement->refresh(); $old=$placement->status;
    ...
    private function authorizeCto(User $user): void { abort_unless($user->roles()->where('slug','cto')->exists(),403,'Only the CTO can manage final placement.'); }
    private function eligibleSupervisors() { return User::query()->where('is_active',true)->where(function($query){$query->whereHas('permissions',fn($q)=>$q->where('slug','applications.assign_supervisor'))->orWhereHas('roles.permissions',fn($q)=>$q->where('slug','applications.assign_supervisor'));})->orderBy('fullname'); }
