...
class AdminWorkflowController extends Controller
{
    ...
    public function show(WorkflowDefinition $workflow)
    {
        ...
        return view('admin.workflows.show',['workflow'=>$workflow,'draft'=>$workflow->versions()->where('status','DRAFT')->latest('version')->with(['stages','transitions.fromStage','transitions.toStage'])->first()]);
    }
    ...
    public function updateVersion(Request $request, WorkflowVersion $version)
    {
        ...
        $data=$request->validate(['stages'=>['required','array','min:1'],'stages.*.name'=>['required','string','max:150'],'stages.*.code'=>['required','string','max:100','alpha_dash'],'stages.*.required_permission'=>['nullable','string','max:150'],'stages.*.is_terminal'=>['nullable','boolean'],'transitions'=>['nullable','array'],'transitions.*.from_stage_code'=>['required','string','max:100'],'transitions.*.to_stage_code'=>['required','string','max:100'],'transitions.*.action'=>['required','string','max:100','alpha_dash'],'transitions.*.label'=>['required','string','max:150'],'transitions.*.required_permission'=>['nullable','string','max:150'],'transitions.*.requires_comment'=>['nullable','boolean']]);
        ...
        DB::transaction(function() use($version,$data){WorkflowTransition::where('workflow_version_id',$version->id)->delete();WorkflowStage::where('workflow_version_id',$version->id)->delete();$stages=[];foreach(array_values($data['stages']) as $index=>$stage)$stages[$stage['code']]=WorkflowStage::create(['workflow_version_id'=>$version->id,'name'=>$stage['name'],'code'=>$stage['code'],'stage_order'=>$index+1,'required_permission'=>$stage['required_permission']??null,'is_terminal'=>!empty($stage['is_terminal'])]);foreach($data['transitions']??[] as $transition)WorkflowTransition::create(['workflow_version_id'=>$version->id,'from_stage_id'=>$stages[$transition['from_stage_code']]->id,'to_stage_id'=>$stages[$transition['to_stage_code']]->id,'action'=>$transition['action'],'label'=>$transition['label'],'required_permission'=>$transition['required_permission']??null,'requires_comment'=>!empty($transition['requires_comment'])]);});
        return back()->with('success','Draft workflow version updated.');
    }
    public function publish(Request $request, WorkflowVersion $version)
    {
        abort_unless($version->status==='DRAFT',422,'Only draft workflow versions can be published.');$version->load(['definition','stages','transitions']);
        ...
        foreach($version->stages->where('is_terminal',false) as $stage)if(!$version->transitions->where('from_stage_id',$stage->id)->count())throw ValidationException::withMessages(['workflow'=>"Stage {$stage->name} has no outgoing transition."]);
