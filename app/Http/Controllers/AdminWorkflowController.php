...
class AdminWorkflowController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.workflows.show', $definition)
            ->with('success', 'Workflow created. Configure the draft before publishing it.');
    }
    ...
    public function show(WorkflowDefinition $workflow)
    {
        $workflow->load([
            'trainingType',
            'versions.stages.responsibleRole',
            'versions.transitions.fromStage',
            'versions.transitions.toStage',
            'versions.transitions.responsibleRole',
        ]);
    ...
    public function updateVersion(Request $request, WorkflowVersion $version)
    {
        ...
        $data = $request->validate([
            ...
            'transitions.*.result_status' => ['required', Rule::in(array_keys(['UNDER_REVIEW' => 1, 'RETURNED' => 1, 'ACCEPTED' => 1, 'REJECTED' => 1]))],
