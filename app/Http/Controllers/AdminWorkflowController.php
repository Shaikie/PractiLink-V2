...
class AdminWorkflowController extends Controller
{
    public function index()
    {
        return view('admin.workflows.index', [
            'workflows' => WorkflowDefinition::with(['trainingType','versions'])->latest()->get(),
        ]);
    }
    ...
    public function show(WorkflowDefinition $workflow)
    {
        ...
        return view('admin.workflows.show', [
            'workflow'=>$workflow,
            'draft'=>$draft,
            'roles'=>Role::orderBy('name')->get(['id','name','slug']),
            'permissions'=>Permission::orderBy('name')->get(['id','name','slug']),
            'actions'=>collect(['FORWARD','RETURN','REJECT','ACCEPT','ASSIGN_SUPERVISOR']),
        ]);
    }
        ...
        DB::transaction(function () use ($version, $data) {
            ...
            foreach (array_values($data['stages']) as $index => $stage) {
                $created = WorkflowStage::create([
                    ...
                    'code'=>$stage['code'],
                    'stage_order'=>$index+1,
                    'required_permission'=>$stage['required_permission'] ?? null,
            ...
            foreach ($data['transitions'] ?? [] as $transition) {
                ...
                WorkflowTransition::create([
                    ...
                    'action'=>strtoupper($transition['action']),
                    'label'=>$transition['label'],
    ...
    public function publish(Request $request, WorkflowVersion $version)
    {
        ...
        if (!$version->stages->where('is_starting',true)->count()) throw ValidationException::withMessages(['workflow'=>'Select a starting stage before publishing.']);
        if ($version->stages->where('is_starting',true)->count() > 1) throw ValidationException::withMessages(['workflow'=>'A workflow can have only one starting stage.']);
